...
export async function AdminActivity() {
  ...
  const recentLedger: LedgerEntry[] = await db.collection(collections.ledger).find({}).sort({ createdAt: -1 }).limit(5).toArray()
  ...
  const recentProducts: Product[] = await db.collection(collections.products).find({}).sort({ updatedAt: -1 }).limit(3).toArray()
